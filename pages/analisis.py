...
def graficar_intervalos_confianza(df, sensores_df, variable, nombre_variable=None):
    ...
    Si la imagen ya existe en assets, la reutiliza.
    """
    if nombre_variable is None:
        nombre_variable = variable

    # Define filename for caching
    filename = f"intervalos_{variable.lower()}.png"
    current_dir = os.path.dirname(os.path.abspath(__file__))
    project_dir = os.path.dirname(current_dir)
    assets_dir = os.path.join(project_dir, "assets")
    filepath = os.path.join(assets_dir, filename)

    # If file exists, return base64 string directly
    if os.path.exists(filepath):
        with open(filepath, "rb") as f:
            img_str = base64.b64encode(f.read()).decode('utf-8')
        return f"data:image/png;base64,{img_str}"

    # Otherwise, generate and save the plot
    ...
    # Read and encode the image to base64 string
    with open(filepath, "rb") as f:
        img_str = base64.b64encode(f.read()).decode('utf-8')
    return f"data:image/png;base64,{img_str}"
